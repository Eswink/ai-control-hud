...
func TestResolveCommandCodeSecretPathUsesMachineConfig(t *testing.T) {
	...
	for _, path := range []string{runtimeDB} {
	...
	if err := machineconfig.Save(configPath, config); err != nil {
		// Windows source-access ACL preparation is exercised elsewhere; this
		// test is about resolving an already persisted SecretStore path.
